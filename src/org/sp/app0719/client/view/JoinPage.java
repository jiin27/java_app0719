...
public class JoinPage extends Page implements ActionListener{
	...
	public void regist() {
		//아이디, 패스워드 등 입력폼의 데이터를 하나의 DTO 담아서 insert 메서드로 전달하자 call by reference
		Member2 member2 = new Member2();
		
		//비어있는 DTO 안에 입력폼의 데이터들을 채워넣기 setter 이용
		member2.setId(t_id.getText()); //아이디 채우기
		member2.setPass(new String(t_pass.getPassword())); //패스워드는 char[] 형식으로 돼 있음. 배열 아님. String 객체 취급되기 때문에 String객체를 호출해서 매개변수로 char[] 형식을 넣어주면 됨!
